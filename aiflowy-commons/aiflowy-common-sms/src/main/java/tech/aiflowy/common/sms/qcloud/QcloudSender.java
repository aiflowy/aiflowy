...
import tech.aiflowy.common.sms.SmsSender;
...
public class QcloudSender implements SmsSender {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(QcloudSender.class);

    @Resource
    private QcloudSmsSenderConfig config;
...
//            req.setExtendCode(extendCode);
            ...
            if (res == null || res.getSendStatusSet() == null || res.getSendStatusSet().length == 0) {
                LOG.error("SendSMS: status failed, res is null");
                return false;
            }

            if (res.getSendStatusSet()[0].getCode().equalsIgnoreCase("ok")) {
                return true;
            } else {
                LOG.error("SendSMS: status failed, " + SendSmsResponse.toJsonString(res));
            }
