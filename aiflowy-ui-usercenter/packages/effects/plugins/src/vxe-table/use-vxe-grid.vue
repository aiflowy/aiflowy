...
import type { AIFlowyFormProps } from '@aiflowy-core/form-ui';
...
import { usePriorityValues } from '@aiflowy/hooks';
import { EmptyIcon } from '@aiflowy/icons';
...
import { AIFlowyHelpTooltip, AIFlowyLoading } from '@aiflowy-core/shadcn-ui';
...
async function init() {
  ...
  // form 由 aiflowy-form代替，所以不适配formConfig，这里给出警告
  const formConfig = gridOptions.value?.formConfig;
  // 处理某个页面加载多个Table时，第2个之后的Table初始化报出警告
  // 因为第一次初始化之后会把defaultGridOptions和gridOptions合并后缓存进State
  if (formConfig && formConfig.enabled) {
    console.warn(
      '[AIFlowy Vxe Table]: The formConfig in the grid is not supported, please use the `formOptions` props',
    );
  }
  props.api?.setState?.({ gridOptions: defaultGridOptions });
  // form 由 aiflowy-form 代替，所以需要保证query相关事件可以拿到参数
...
watch(
  ...
  () => {
    formApi.setState((prev) => {
      const finalFormOptions: AIFlowyFormProps = mergeWithArrayOverride(
        {},
        formOptions.value,
        prev,
      );
      ...
      <template v-if="showToolbar" #toolbar-actions="slotProps">
        <slot v-if="showTableTitle" name="table-title">
          <div class="mr-1 pl-1 text-[1rem]">
            {{ tableTitle }}
            <AIFlowyHelpTooltip v-if="tableTitleHelp" trigger-class="pb-1">
              {{ tableTitleHelp }}
            </AIFlowyHelpTooltip>
          </div>
        </slot>
